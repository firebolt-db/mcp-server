...
func TestNewCorePool(t *testing.T) {

	t.Run("create new pool", func(t *testing.T) {
		pool, closer := database.NewCorePool(slogt.New(t), "http://localhost:3744")
	...
	t.Run("create pool with custom connection factory", func(t *testing.T) {
		...
		pool, closer := database.NewCorePoolWithConnectionFactory(slogt.New(t), "http://localhost:3744", customFactory)
		require.NotNil(t, pool, "Pool should not be nil")
		require.NotNil(t, closer, "Closer function should not be nil")
...
func TestCorePoolGetConnection(t *testing.T) {
	...
	t.Run("get different connections", func(t *testing.T) {
		...
		pool, poolCloser := database.NewCorePoolWithConnectionFactory(slogt.New(t), "http://localhost:3744", customFactory)
		t.Cleanup(poolCloser)

		// Create connections with different parameters
		params1 := database.PoolParams{
			DatabaseName: strPtr("database1"),
		}

		params2 := database.PoolParams{
			DatabaseName: strPtr("database2"),
		}
...
func TestCorePoolClose(t *testing.T) {
	...
	t.Run("close all connections", func(t *testing.T) {
		...
		params1 := database.PoolParams{DatabaseName: strPtr("database1")}
		params2 := database.PoolParams{DatabaseName: strPtr("database2")}
	...
	t.Run("multiple close calls", func(t *testing.T) {
		...
		// Create a connection
		params := database.PoolParams{DatabaseName: strPtr("database")}
...
func TestCorePoolConcurrency(t *testing.T) {
